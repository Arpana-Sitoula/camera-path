...
# We only removed the global feature extractor and reordered some definitions of layers.
# This does not change the network architecture, but simplifies the LRP analysis.
###########################################################################
...
class ContextGuidedBlock(nn.Module):
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16, add=True):
        ...
        self.add = add
        self.add_mod = net_mod.AddConnectionConcat()
        self.cat = net_mod.SkipConnectionConcat()
    ...
    def forward(self, input):
        ...
        # if residual version
        if self.add:
            output = self.add_mod([input, output])
        return output
...
class CGNet(nn.Module):
    ...
    def __init__(self, classes=3, channels=4, M=3, N=21, dropout_flag=False):
        """
        args:
        ...
        if dropout_flag:
            ...
            self.classifier = nn.Sequential(Conv(256, classes, 1, 1), nn.ReLU())
